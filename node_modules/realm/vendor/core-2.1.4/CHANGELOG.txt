...
-   Deleting rows through a TableView generated wrong instructions by
    ...
    reapplying the transaction log to a separate Realm file or
    via synchronization.
...
-   Removed ("deleted") the default copy constructor for RowBase. This
    ...
    data race. Said race was benign, but would be reported by the
    thread sanitizer.
...
-   BREAKING! Until now, a Query would return indexes into a restricting
    ...
    or it would return indexes into the Table if no restricting view
    was present. This would make query results useless if you did not
    know whether or not a restricting view was present. This fix make it
...
-   Bug fixed in test harness: In some cases some tests and checks would
    be counted twice due to counters not being reset at all the
    right times.
...
-   The commit logs have been moved into the Realm file. This means we
    ...
    interoperate with earlier versions which still uses separate
    .log files. (issues #2065, #1354).
...
-   Fixed java bug #3144 / Core #2014. Management of Descriptor class
    ...
    on one thread, while new TableViews where created on a
    different thread.
...
-   Fix crashes and general brokenness when constructing a Query,
    inserting a column into the queried table, and then running
    the query.
-   Fix crashes and general brokenness when syncing a sorted or
    distincted TableView after inserting new columns into the
    source Table.
...
-   Reduce the number of file descriptors used in robust mutex
    emulation, multi instances of InterprocessMutex share the
    same descriptor. (#1986)
...
-   Increased the verbosity of some exception error messages to
    help debugging.
...
-   Update table views so that rows are not attached after
    calling Table::clear() (#1837)
...
-   The slabs (regions of memory used for temporary storage during a
    ...
    the allocator was detached, the file shrunk and the allocator
    was re-attached. This scenario can be triggered by compact, or by
    ...
    overlapping memory chunks, one of which would then later corrupt
    the other. To a user this would look like file corruption. It is
